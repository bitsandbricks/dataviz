...
ggplot(filter(gapminder, año == 2007), aes(x = PBIpc, y = expVida, size = pobl, color = continente)) +
  geom_point() +
  scale_x_log10()
...
ggplot(filter(gapminder, año == 2007), aes(x = PBIpc, y = expVida, size = ___, color = continente)) +
  geom_point() +
  scale_x_log10()
```

```{r plotminder6-hint}
ggplot(filter(gapminder, año == 2007), aes(x = PBIpc, y = expVida, size = pobl/1______, color = continente)) +
  geom_point() +
  scale_x_log10()
```
...
ggplot(filter(gapminder, año == 2007), aes(x = PBIpc, y = expVida, size = pobl/1000000, color = continente)) +
  geom_point() +
  scale_x_log10()
...
```{r}
color_continentes <- list("Europe" = "darkorange", "Asia" = "red", "Africa" = "blue", 
...
Incluso podemos definir a mano cada componente visual de los gráficos, creando así nuestro propio tema. No vamos a entrar en detalles ahora, pero para quien tenga interés el camino comienza por el uso de la [función `theme`](https://ggplot2.tidyverse.org/reference/theme.html). También existen paquetes de R que agregan a nuestro arsenal nuevos temas listos para usar, como [`ggthemes`](https://ggplot2.tidyverse.org/reference/theme.html) y [`hrbrthemes`](https://cinc.rud.is/web/packages/hrbrthemes/). 

Para cerrar el tema de los temas (¡ja!) va una recomendación. El tema minimalista "theme_minimal" viene con `ggplot2` por lo que ya lo tenemos disponible, y con el mínimo esfuerzo de escribir la línea que lo aplica obtenemos un gráfico de aspecto más "limpio" que el del tema por default:

```{r plotminder9, include=TRUE}
ggplot(filter(gapminder, año == 2007), aes(x = PBIpc, y = expVida, size = pobl/1000000, color = continente)) +
  geom_point() +
  scale_x_log10() +
  scale_colour_manual(values = color_continentes) + 
